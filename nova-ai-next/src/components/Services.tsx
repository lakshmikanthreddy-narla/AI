  ...
  return (
    ...
    >
      ...
      >
        {/* FRONT SIDE */}
        <div
          className={`absolute inset-0 w-full h-full p-6 rounded-2xl border flex flex-col justify-between overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 ${index === 0
            ? "bg-[rgba(56,189,248,0.3)] border-blue-300/80 text-black"
            : "bg-white border-zinc-200/85 text-black"
            }`}
          style={{
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div>
              <div className="flex justify-between items-center">
                <span className="font-mono text-zinc-300 text-3xl font-black">{service.id}</span>
                <span className="text-[9px] uppercase tracking-wider font-bold bg-zinc-100 text-zinc-550 px-2.5 py-1 rounded border border-zinc-200/50">
                  {service.title}
                </span>
              </div>
              ...
              <div className="flex items-center gap-1 mt-4 text-xs font-bold text-zinc-400 uppercase tracking-wider">
                <span>Flip Card</span>
            ...
            {service.image && (
              <div className={`absolute bottom-0 left-0 right-0 h-[210px] rounded-b-2xl overflow-hidden flex items-center justify-center bg-transparent ${index === 0 ? "p-0" : "p-2"
                }`}>
                <img
                  src={service.image.src}
                  alt={service.title}
                  className={`w-full h-full ${index === 0 ? "object-cover mix-blend-screen" : "object-contain scale-95 invert opacity-90"
                    }`}
                />
              </div>
            )}
        ...
        {/* BACK SIDE */}
        <div
          className={`absolute inset-0 w-full h-full p-6 rounded-2xl border flex flex-col justify-between overflow-hidden shadow-sm ${index === 0
            ? "bg-[rgba(56,189,248,0.3)] border-blue-300/80 text-black"
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            {/* Top Details */}
            <div>
              <span className="text-[9px] uppercase tracking-wider font-bold bg-blue-50 text-blue-600 px-2.5 py-1 rounded border border-blue-100 inline-block">
                {service.title}
              </span>
            ...
            <div className="my-3">
              <p className="text-[9px] uppercase tracking-widest text-zinc-400 mb-2 font-bold">
                KEY FOCUS AREAS
              </p>
              <div className="flex flex-wrap gap-1.5">
                {service.includes.map((inc) => (
                  <span
                    key={inc}
                    className="text-[10px] bg-zinc-50 border border-zinc-200 px-2.5 py-1 text-zinc-700 font-semibold rounded"
                  >
                    {inc}
                  </span>
                ))}
            ...
            <div className="mt-auto pt-2 border-t border-zinc-100">
              <a
                href="#contact"
                className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-black border border-zinc-300 hover:bg-black hover:text-white transition-all px-4 py-2 rounded"
              >
                <span>Know more</span>
